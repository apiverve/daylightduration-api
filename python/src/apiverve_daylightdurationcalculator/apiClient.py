...
class DaylightdurationAPIClientError(Exception):
...
class ValidationError(DaylightdurationAPIClientError):
...
class DaylightdurationAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"latitude": {"type": "number", "required": True, "min": -90, "max": 90}, "longitude": {"type": "number", "required": True, "min": -180, "max": 180}, "date": {"type": "string", "required": False, "format": "date", "default": "today"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the DaylightdurationAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises DaylightdurationAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/daylightduration'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise DaylightdurationAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[DaylightdurationAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns DaylightDurationCalculatorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises DaylightdurationAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise DaylightdurationAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise DaylightdurationAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise DaylightdurationAPIClientError(f"Request failed: {str(e)}")
